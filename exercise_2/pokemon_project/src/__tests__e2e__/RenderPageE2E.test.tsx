...
describe("Render APP", () =>{
    ...
    it("Ensure name is displayed",async () => {
        await page.waitForSelector("#pokemonName")
        let pokeName = await page.$eval("#pokemonName", (name)=>name.innerHTML)

        expect(pokeName.length).toBeGreaterThan(0)
    })

    it("Ensure img is displayed", async() =>{
        await page.waitForSelector("#pokemonIMG")
        let pokemonImg = await page.$("#pokemonIMG")
